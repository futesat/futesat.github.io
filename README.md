# Tech Lead / Software Engineer Portfolio

This is a modern, responsive portfolio website formatted for GitHub Pages. It features a "2025" aesthetic with dark mode, glassmorphism, and subtle animations.

## Live Demo
Check it out at: [https://futesat.github.io](https://futesat.github.io)

## Features
- **Modern Design**: Deep dark styling with neon accents (`#6d28d9`, `#00d4ff`).
- **Responsive**: Fully optimized for Mobile, Tablet, and Desktop.
- **Glassmorphism**: Premium frosted glass effects on cards and navigation.
- **Interactive**: Custom cursor glow and scroll-reveal animations.
- **Micro-interactions**: Smooth hover states and transitions.

## Tech Stack
- **HTML5**: Semantic structure.
- **CSS3**: Variables, Flexbox, Grid, Animations.
- **JavaScript (Vanilla)**: Intersection Observers for performance-friendly animations.
- **FontAwesome**: Icons (via CDN).
- **Google Fonts**: Space Grotesk (Headers) & Outfit (Body).

## How to Edit
1. **Modify Content**: Edit `index.html`. All text sections (About, Experience, Skills) are clearly marked.
2. **Change Colors**: Open `styles.css` and adjust the `:root` variables at the top.
   ```css
   :root {
       --accent-primary: #YOUR_COLOR;
       --accent-secondary: #YOUR_COLOR;
   }
   ```
3. **Images**: Replace or add images in an `/assets` folder and reference them in HTML.

## Deployment
Since this is a GitHub Pages repository, simply push your changes to the `main` or `master` branch. GitHub will automatically deploy these static files.

---
Built by [Cesar Beiro](https://github.com/futesat)
